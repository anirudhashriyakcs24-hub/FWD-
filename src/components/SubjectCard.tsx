...
/** ✅ Explicitly allowed subjects */
type SubjectSlug = 'physics' | 'chemistry' | 'mathematics';
...
const SubjectCard = ({ name, slug, description, topicCount = 5 }: SubjectCardProps) => {
  ...
  const cardClass = colorMap[slug];
  const bgClass = bgColorMap[slug];
  const iconClass = iconColorMap[slug];
